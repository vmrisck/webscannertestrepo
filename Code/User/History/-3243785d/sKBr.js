...
/*     sql = 'SELECT id, tabId FROM domain WHERE jobStatus !=?'
    ...
    const execCmd = `timeout 5 /usr/bin/google-chrome --no-first-run --user-data-dir=$(mktemp -d /project/riscosity/webscanner/data/extension/temp.XXXXXX) --load-extension='/project/riscosity/webscanner/extension'  'https://support.google.com/chrome/?hl=en&scanJobId=67&isPageLink=1' 2>>/project/riscosity/webscanner/data/logs/webscanner.log`
...
app.get('/getAllTraffic', async (req, res) => {
    ...
    for (let row of sqlResults) {
        ...
        if (trackerUrl.length) {
            ...
            if (hostname && !trafficStats.trackerDomainMap?.[trackerHostPart])
                trafficStats.trackerDomainMap?.[trackerHostPart] = []
            if (!trafficStats.trackerDomainMap[trackerHostPart].includes(nameHostPart))    
                trafficStats.domainTrackerMap[trackerHostPart].push(nameHostPart)
        } 
        ...
        } else if (hostname && isBareIP(hostname) && !trafficStats.ipList.includes(hostname))
