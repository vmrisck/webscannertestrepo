...
app.get('/getAllTraffic', async (req, res) => {
    ...
    for (let row of sqlResults) {
        ...
        traffic = {     // Create traffic obj
            ...
            redirectUrl: redirectUrl?.substring(0,50), // TODO: Truncated temporarily
            trafficUrl: trafficUrl?.substring(0,50),
        };

        let newNode = {};  // Create newNode
        newNode.domainId = domainId
        newNode.parentDomainId = parentDomainId
        newNode.name = name.substring(0,50) // TODO: Truncated temporarily
...
//clog(`domainTraffic: `, domainTraffic)
    ...
    const execCmd = `/project/riscosity/webscanner/scanner/domainContentScan.sh ${domainTraffic.name} 2>>/project/riscosity/webscanner/data/logs/webscanner.log`
    clog(`scanner cmd: ${execCmd}`)
    try {
        const output = execSync(execCmd);
        console.log(JSON.parse(output).data)
        trafficStats.vulnerabilities = JSON.parse(output).data
    } catch (e) { console.error(e) }
