        ...
        try {
            ...
            for (const item of repoStats.split('\n')) {
                ...
                scanResults.recentRepos.forEach(recent => {
                    ...
                    let repoNameHtml = ''
                    if (recent.repoUrl)
                        repoNameHtml = `<a style="color: gold; overflow: hidden;" href="${recent.repoUrl}">${recent.repoName}</a>`
                    else
                        repoNameHtml = recent.repoName
                    recentRow += `<div class="table-body-cell" style=" inline-size: 200px;">${repoNameHtml}</div>`
