        ...
        try {
            ...
            for (const item of repoStats.split('\n')) {
                ...
                scanResults.recentRepos.forEach(recent => {
                    ...
                    let repoNameHtml = ''
                    if (recent.repoUrl)
                        repoNameHtml = `<a style="color: gold" href="${recent.repoUrl}">${recent.repoName}</a>`
                    else
                    ...
                    if (recent.repoPiiCount)
                        recentRow += `<div class="table-body-cell"><img src="img/check-circle.svg" alt="${recent.repoPiiCount}" class="piiFound"/></div>`
                    else 
                        recentRow += `<div class="table-body-cell"><img src="img/x-circle.svg" alt="${recent.repoPiiCount}" class="piiFound"/></div>`


                    if (recent.repoApiCallsCount)                        
                        recentRow += `<div class="table-body-cell"><img src="img/check-circle.svg" alt="${recent.repoApiCallsCount}" class="piiFound"/></div>`
                    else
                        recentRow += `<div class="table-body-cell">${recent.repoApiCallsCount}</div>`
    ...
    for (const key of Object.keys(piiIconMap)) {
        if (text.includes(key)) {
            if (piiIconMap[key]?.type == "image")
                htmlText = `<img src="img/${piiIconMap[key]?.value}" class="piiIcon"/>&nbsp;`
    ...
    while (true) {
        ...
        for (const item of logList.split('\n')) {
            ...
            for (const line of logFile.split('\n')) {
                ...
                let midPoint = Math.trunc(Object.keys(scanResults.currentRepo).length / 2);
                let repoIndex = 0, repoStat1 = {}, repoStat2 = {};
                for (const key in scanResults.currentRepo) {
                    //console.log(`${property}: ${object[property]}`);
                    if (key == 'fileName') continue;
                    if (++repoIndex <= midPoint)
                        repoStat1[key] = scanResults.currentRepo[key]
                    else
                        repoStat2[key] = scanResults.currentRepo[key]
                ...
                document.getElementById("scanResultsRight").innerHTML = JSON.stringify(repoStat2, null, 2);
            }
        }
    }
})();
