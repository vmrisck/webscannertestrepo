const apiBaseUrl = "http://18.220.95.222:8081";
        ...
        try {
            ...
            for (const item of repoStats.split('\n')) {
                ...
                scanResults.recentRepos.forEach(recent => {
                    ...
                    let repoNameHtml = ''
                    if (recent.repoUrl) 
                        repoNameHtml = `<a href="${recent.repoUrl}">${recent.repoName}</a>`
                    else 
                        repoNameHtml = recent.repoName
                    recentRow += `<div class="table-body-cell">${repoNameHtml}</div>`

                    if (recent.repoType == 'github')
                        recentRow += `<div class="table-body-cell"><img src="img/github-mark.png" class="octicon"/></div>`
                        //recentRow += `<div class="table-body-cell"><img src="img/GitHub_Logo.png" class="octicon-logo"/></div>`
                    else if (recent.repoType == 'npm')
                        recentRow += `<div class="table-body-cell"><img src="img/npm.svg" class="npmicon"/></div>`
                    else
                    ...
                    recentRow += `<div class="table-body-cell">${recent.repoPiiCount}</div>`
                    ...
                    recentRow += `<div class="table-body-cell">${recent.repoPiiTypes}</div>`
    ...
    while (true) {
        ...
        for (const item of logList.split('\n')) {
            ...
            for (const line of logFile.split('\n')) {
                ...
                document.getElementById("scanResults").innerHTML = JSON.stringify(scanResults.currentRepo, null, 2);
            }
        }
    }
})();
