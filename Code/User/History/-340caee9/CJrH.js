...
document.onreadystatechange = () => {
  if (document.readyState === 'complete') {
    ...
    for (let i = 0; i < anchorTags.length && i <= maxScanPageLinks ; i++) {
    ...
    setTimeout( () => {
        // allow time for prev requests to process
        //await new Promise(r => setTimeout(r, 5000)); 
        sendRequest(`http://localhost:8000/pageLoaded?scanJobId=${scanJobId}&pageLinks=${JSON.stringify(pageLinks)}`)
        //window.close()
    }, 4000); 
  }
};
