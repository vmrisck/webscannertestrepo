...
const maxScanJobRuntime = 15;
const maxScanPageLinks = 5;
var tabUrl;
...
try {
    ...
    if (scanJobId = (new URLSearchParams(winLocation.search)).get('scanJobId')) 
        sendRequest(`${apiEndpoint}/contentScriptLoading?scanJobId=${scanJobId}`)
...
document.onreadystatechange = () => {
  if (document.readyState === 'complete') {
    ...
    try {
        scanJobId = (new URLSearchParams(winLocation.search)).get('scanJobId')
    ...
    setTimeout( () => {
        console.log(`Sending pageLinks `, pageLinks);
        sendRequest(`${apiEndpoint}/pageLoaded?scanJobId=${scanJobId}&pageLinks=${pageLinks}`)
    }, 2000);

    setTimeout( () => {
        window.close()    // TODO: Enabling win close prevents traffic logging?
    }, (maxScanJobRuntime - 1) * 1000);
  }
};
