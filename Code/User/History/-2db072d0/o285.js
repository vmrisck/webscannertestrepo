...
chrome.webRequest.onBeforeRequest.addListener(async details => {
    ...
    if (tabId >= 0) {
     ...
     (async () => {
      ...
      if (reqUrl == `${apiEndpoint}/logTraffic`)
        return
      try {
        let fetchBody = {
            winLocation: encodeURIComponent(JSON.stringify(winLocation)),
            url: reqUrl,
            method: reqMethod,
            //details: details,
            responseStatus: '',
        ...
        const response = await fetch(`${apiEndpoint}/logTraffic?listner=onBeforeRequest`, {
            method: 'POST',
...
chrome.webRequest.onBeforeSendHeaders.addListener( async details => {
        ...
        let reqUrl = details.url
        let myRequestHeaders = JSON.stringify(details.requestHeaders, null, 2);
        let tabId = details?.tabId;
        let requestId = details?.requestId
        let winLocation = ''
        ...
        (async () => {
            ...
            try {
                ...
                const response = await fetch(`${apiEndpoint}/logTraffic?listner=onBeforeSendHeaders`, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify(fetchBody)
                });
...
chrome.webRequest.onHeadersReceived.addListener( async details => {
        ...
        if (tabId < 0 || reqUrl.includes(`${apiEndpoint}/logTraffic?listner=onHeadersReceived`))
            return {cancel: false};
        ...
        (async () => {
            try {
                ...
                //console.log('fetchBody is : ', fetchBody);
                const response = await fetch(`${apiEndpoint}/logTraffic`, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify(fetchBody)
                });
