...
//const scanJobIdFile = require('./scanJobId.txt');

importScripts('./scanJobId.txt')
...
chrome.webRequest.onBeforeRequest.addListener(async details => {
    ...
    let reqBody = (reqMethod == "POST") ? JSON.stringify(details?.requestBody) : "";

/*     if (reqMethod == "POST" && details?.requestBody) {
        reqBody = JSON.stringify(JSON.parse(details?.requestBody), (key, value) => {
    ...
    if (tabId >= 0) {
     ...
     (async () => {
      ...
      try {
        let fetchBody = {
            extnScanJobId: scanJobIdFile.extnScanJobId || 55,
...
/*         let myRequestHeaders = JSON.stringify(JSON.parse(details.requestHeaders), (key, value) => {
        ...
        (async () => {
            ...
            try {
                let fetchBody = {
                    extnScanJobId: scanJobIdFile.extnScanJobId || 56,
                    winLocation: winLocation,
                    url: reqUrl,
...
chrome.webRequest.onHeadersReceived.addListener( async details => {
        ...
        (async () => {
            ...
            try {
                let fetchBody = {
                    extnScanJobId: scanJobIdFile.extnScanJobId || 57,
                    winLocation: winLocation,
                    url: reqUrl,
                    requestMethod: details.method,
