...
app.get('/listLogs', (req, res) => {
    ...
    // TODO: shuf | exclude repostats.txt and -stats.log
    exec(`find ${baseDir}/data/logs/scan -mindepth 2 -type f -printf '%P\n'`, {maxBuffer: 1024 * 5000}, (error, stdout, stderr) => {
        if (error) {
...
// Return repo specific stats file 
app.get('/listRepoStats', (req, res) => {
   exec(`find ${baseDir}/data/logs/scan -name '*-stats.log' -printf '%T@\t%P\n' | sort -rn | cut -f 2`, {maxBuffer: 1024 * 5000}, (error, stdout, stderr) => {
       if (error) {
           console.log(`error: ${error.message}`);
           return;
       }
